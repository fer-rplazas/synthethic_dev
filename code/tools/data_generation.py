...
class DataGenerator:
    def __init__(
        ...
        """Object that generates a signal that probabilistically transitions between two states whose features are defined by a configuration. Data is generated on __init__.

        Args:
            cfg (dict[str,list[float]|None]): Configuration object defining whether signal features are included and how their intensity differs across states.
            snr (float, optional): Signal-to-noise ratio (signal power / noise power), noise is 1/f additive colored noise. Defaults to 10.
